...
function MainLayout ({ Routes }) {
  ...
  useEffect(() => {
    ...
    const loadScrollTop = async () => {
      const importedModule = await import('/imports/ui/components/Common/ScrollTop')
      ScrollTop = importedModule.default
    }

    if (!isTablet && (typeof window !== 'undefined' ? window.innerWidth > 1280 : true) && !ScrollTop) {
      loadScrollTop()
    }
  ...
  const renderCookieBar = () => {
    return (<div id='cookies-bar-id' className={isTablet ? 'cookies-bar padding-bottom' : 'cookies-bar'}>
  ...
  const renderSignupBar = () => {
    return (<div id='cookies-bar-id' className={isTablet ? 'cookies-bar signup-bar padding-bottom' : 'cookies-bar signup-bar'}>
