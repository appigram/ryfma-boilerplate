...
async function getPartialResponse(req, res) {
...
self.addEventListener('fetch', (event) => {
  if (!event.request.headers.get('range')) {
    ...
    const requestToFetch = event.request;
    return fetch(requestToFetch).then((response) => {
      return event.respondWith(getPartialResponse(event.request, response));
    });
  }
});
