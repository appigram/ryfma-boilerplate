...
import connectRoute from 'connect-route-ext'
...
const uploadRequest = Meteor.bindEnvironment(async function (context, data) {
  const { directiveName, file, meta } = JSON.parse(data)
  ...
  if (!directive.requestAuthorization(context, file, meta)) {
    throw new Meteor.Error("Unauthorized", "You are not allowed to " +
      "upload this file");
  }

  const response = directive.getInstructions(context, file, meta);
  return response
})

WebApp.connectHandlers.use(connectRoute(function (router) {
  router.post('/methods/slingshot/uploadRequest', (req, res, next) => {
    let data = '';
    req.on("data", chunk => {
      data += chunk;
    });
    req.on('end', async () => {
      const response = await uploadRequest(this, data);
