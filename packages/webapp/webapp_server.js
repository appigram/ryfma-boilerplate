    ...
    if (!appUrl(req.url)) {
      ...
      return getBoilerplateAsync(request, arch).then(({ stream, statusCode, headers: newHeaders, }) => {
        ...
        if (request?.path?.startsWith('/amp/')) {
          // const ampOptimizer = AmpOptimizer.create();
          streamToString(stream).then(function (html) {
            // console.log('process AMP HTML')
            // console.log('Orig HTML: ', html)
            const cleanHTML = html
              .replace(/\sxmlns[^"]+"[^"]+"/gi, '')
              .replace(/\sxml\:lang[^"]+"[^"]+"/gi, '')
              .replace(/<script type="text\/javascript">__meteor_runtime_config__.*<\/script>/gi, '')
              .replace(/<script.*meteor_js_resource=true"([^>]*?)><\/script>/gi, '')
              .replace(/<script.*\/packages\/([^>]*?)><\/script>/gi, '')
              .replace(/<script.*\/app\/([^>]*?)><\/script>/gi, '')
              .replace(/<link.*class="__meteor-css__"([^>]*?)>/gi, '')
              .replace(/<br([^>]*?)>/gi, '<br/>')
              .replace(/<img(.*?)\/?>/gi, '<amp-img$1 layout="fill" i-amphtml-layout="fill" data-amp-auto-lightbox-disable></amp-img>')
              .replace(/\sloading="lazy"/gi, '')
              .replace(/\sdecoding="async"/gi, '')
              .replace(/!important/gi, '')
              .replace(/<iframe/gi, '<amp-youtube layout="responsive" ')
              .replace(/<\/iframe/gi, '</amp-youtube')

            const opts = {
              // canonical: request.path,
            }
            ampOptimizer.transformHtml(cleanHTML, opts).then((optimizedHtml) => {
              // console.log(optimizedHtml)
              res.write(optimizedHtml);
              res.end();
              stream.destroy();
            }).catch(error => {
              Log.error("Error getting AMP HTML: " + error.stack);
              res.writeHead(500, headers);
              res.end();
              stream.destroy();
            })
          }).catch(error => {
            Log.error("Error converting Stream to String: " + error.stack);
            res.writeHead(500, headers);
            res.end();
            stream.destroy();
          })
        ...
        Log.error("Error headers: " + headers);
        res.writeHead(500, headers);
        res.end();
      });
    }
